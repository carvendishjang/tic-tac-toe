...
import Control.Concurrent (MVar, newMVar, tryTakeMVar, putMVar, newEmptyMVar)
...
-- Create a window and start working on our game
...
-- We create a new window with a title, our initial state of an empty board with
-- "X" is the next to play, and "azure" as the background color

-- drawBoard returns a Picture, which can be desplayed by gloss. 
-- Pictures with composition form a Monoid; we can easily combine pictures
